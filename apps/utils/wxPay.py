...
class WxPayConf_pub(object):
    ...
    # 受理商ID，身份标识
    MCHID = consts.WX_APP_MCH_ID
    # 商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    KEY = consts.WX_APP_KEY

    # =======【异步通知url设置】===================================
    # 异步通知url，商户根据实际开发过程设定
    NOTIFY_URL = "https://www.zisai.net/api/wxapp/jf/pay/notify/"
    ...
    # =======【证书路径设置】=====================================
    # 证书路径,注意应该填写绝对路径
    SSLCERT_PATH = "/media/cacert/apiclient_cert.pem"
    SSLKEY_PATH = "/media/cacert/apiclient_key.pem"
...
class Common_util_pub(object):
    ...
    def postXmlCurl(self, xml, url, second=30):
        ...
        return HttpClient().postXml(xml.encode('utf-8'), url, second=second)
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        ...
        self.parameters["appid"] = WxPayConf_pub.APPID  # 公众账号ID
        self.parameters["mch_id"] = WxPayConf_pub.MCHID  # 商户号
        self.parameters["spbill_create_ip"] = "218.11.132.35"  # 终端ip
        self.parameters["nonce_str"] = self.createNoncestr()  # 随机字符串
        self.parameters["sign"] = self.getSign(self.parameters)  # 签名
        return self.arrayToXml(self.parameters)
